...
public abstract class RowToObject<D extends RowToObjectDataSource, Res> {
	...
	public RowToObject(Map<String, Integer> headers, final List<String> row, final D dataSource) {
		...
		Integer maxValueSet = Collections.max(valuesSet);
		if ((row.size() - 1) < maxValueSet) {
			throw new IllegalArgumentException("la map de headers fait référence à un index trop grand (" + maxValueSet +
